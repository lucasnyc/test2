...
export class JSValue {
  public readonly isJSValue = true;
  ...
  toString(): string {
    if (this.value && typeof this.value.toReplString === 'function') {
      return this.value.toReplString();
    }
    // Fallback for values without a custom string representation
    if (this.isFunction) {
      return `<function ${this.name}>`;
    }
    // Default for other wrapped values.
