 ...
 */
export function PyCSEResultPromise(result: Result): Promise<Result> {
  return new Promise((resolve, reject) => {
    ...
    const representation = new Representation(finalValue);
    resolve({ status: 'finished', context: result.context, value: finalValue, representation });
  });
}
...
export function PyEvaluate(
  ...
  try {
    ...
    const valueFromMachine = pyRunCSEMachine(
      code,
      context,
      context.control,
      context.stash,
    ...
    if (context.errors.length > 0) {
      return { status: 'error', context: context };
    }

    return {
      status: 'finished',
      context: context,
      value: context.output ? { type: 'string', value: context.output } : valueFromMachine,
      representation: new Representation(valueFromMachine)
    };
  } catch (error: any) {
...
const pyCmdEvaluators: { [type: string]: CmdEvaluator } = {
  ...
  Variable: (code, command, context, control, stash, isPrelude) => {
    ...
    let value = pyGetVariable(code, context, name, variableNode);

    if (isJSValue(value) && !value.isFunction) {
      value = value.value;
    }
  ...
  [InstrType.APPLICATION]: (code, command, context, control, stash, isPrelude) => {
    ...
    if (callable instanceof PyClosure) {
      ...
      if (callable.isFunction) {
        const result = callable.value(context, ...args);
        stash.push(result);
      } else {
