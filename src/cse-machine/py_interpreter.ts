  ...
  return new Promise((resolve, reject) => {
    if (value instanceof CSEBreak) {
      ...
      if (isJSValue(value)) {
        ...
        // Check if the unwrapped value has its own toReplString (e.g., Rune )
        if (unwrapped && typeof unwrapped.toReplString === 'function') {
          stringRepresentation = unwrapped.toReplString();
          resultValue = {
            type: 'result',
            value: unwrapped  // Pass the raw object for frontend rendering
          };
        } else {
          // Fallback for values without toReplString
          stringRepresentation = toPythonString(value);
          resultValue = {
            type: 'result',
            value: unwrapped
          };
  ...
  [InstrType.APPLICATION]: (code, command, context, control, stash, isPrelude) => {
    ...
    if (callable instanceof PyClosure) {
      ...
      if (closureNode.constructor.name === 'FunctionDef') {
        ...
        const result = callable.value.apply(null, marshalledArgs)
