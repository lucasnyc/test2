...
export function bundleAndBind(context: PyContext, linkerResult: LinkerResult): void {
  ...
  imports.forEach((importedItems, moduleName) => {
    ...
    importedItems.forEach(({ name, alias }) => {
      ...
      if (bindingName === 'heart') {
        console.log('--- bundleAndBind ---');
        console.log('Binding name:', bindingName);
        console.log('Value being stored:', jsValue);
        console.log('Type of value being stored:', typeof jsValue);
        console.log('Constructor of value:', jsValue?.constructor?.name);
      }
